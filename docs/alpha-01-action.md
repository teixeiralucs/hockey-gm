...
- **Acompanhamento Fracionado (UI)**: Um *Badge* conta ativamente o progresso de coleção (ex: 7 / 23 Collected) posicionado no cabeçalho da tabela de time atual, atrelado a um contador geral de jogadores salvos no topo direito da HUD.
- **Engine de Premiação Escalável (FPHL Boost)**: Adicionado a rotina `checkTeamCompletion()` nos ganchos de Aposentadoria Automática e Venda para Coleção. Se todos os IDs originais de um time habitarem a coleção simultaneamente, o sistema sinaliza conclusão e presenteia o jogador imediatamente abrindo um *Reveal Modal*.
...
- **Season Awards & Hall of Fame**: Desenvolvida a lógica visual e computacional de premiações de fim de temporada com base na matemática oficial da liga. Através da função `computeSeasonAwards`, o sistema varre a liga calculando os maiores recordes e entregando troféus nomeados autênticos da OHL (ex: *Red Tilson Trophy*, *Max Kaminsky Trophy*, *J. Ross Robertson Cup*). O histórico é salvo de maneira perene no array `gameState.history` e renderizado numa nova e belíssima aba de "Hall of Fame" localizada no Sidebar.
...
- **Correção de Vazamento Estático (Stats Reset)**: Solucionado um erro crítico de *Data Persistence* que quebrava as tabelas de *League Leaders*. Os stats originais da liga de 2023-2024 que acompanhavam os arrays do Scraper agora são instantaneamente formatados em brancos matemáticos (zeros) tanto no `startGame` (criação da franquia) quanto em `advanceSeason`. Para solidificar o patch, um fix isolou `gameState.globalDraftPool` no armazenamento local (`localStorage`), impedindo que partidas simuladas no gelo percam progressão orgânica da IA ao atualizar a aba do navegador.

### Iteração Alpha 0.1.18 (Advanced Save System & Persistence)
- **Slots de Save Múltiplos**: A arquitetura de *Save* foi reescrita abandonando a variável única do `localStorage` para suportar 3 slots manuais (`hockeyGmSave_1`, `_2`, `_3`) e um slot automático (`_auto`), garantindo que o progresso do usuário não seja sobreposto por engano.
...
- **UI Modals para Gerenciamento de Estado**: Injeção de Modais Glassmorphism escuros tanto para salvar quanto para carregar a franquia. A tela de *Load* lista perfeitamente as metadados do save (Nome da Equipe e Ano da Temporada) evitando *blind loads*, além de indicar formatações corrompidas e tratar slots vazios visualmente. E também foi corrigido o crash visual (`currentTeam`) de re-injeção do state durante o carregamento das variaveis do HTML.
- **Exportação/Importação de JSON (File Handling)**: Uma solução radical foi implementada para evitar a perda de Saves pelo ecossistema de origens (`origins`) atrelado a portas no Live Server: a interface de *Load/Save* expõe File Inputs (`<input type="file">`) combinados à API de `Blob`. Agora o GM pode exportar toda a sua franquia para um arquivo `.json` baixado fisicamente na máquina, assim como importá-lo novamente recuperando o jogo perfeitamente.
