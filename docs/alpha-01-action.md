...
- **Fim das Caixas Nativas (Alert/Confirm)**: Substituição radical de todos os eventos síncronos de `window.alert` e `window.confirm` do navegador para Modais Assíncronos Customizados da interface premium (Vender Jogador, Mandar para a Coleção, Falta de Moedas, Limite de Elenco, Draft Esgotado). Essa mudança previne bugs no ciclo de vida nativo do _Drag and Drop_ HTML5 e elimina comportamentos de elementos "fantasmas".
- **Reveal de Pacotes Múltiplos**: Adicionada a função `openPackRevealModal` que abre a animação de sorteio com os 3 cards do pacote injetados num modal Flexbox com microanimações de zoom, melhorando absurdamente o "feel" de _opening_ do sistema de gacha.
...
- **Trilha Lógica Física**: A Engine simula perfeitamente transições de zona (Dump & Chase, Entradas), Rebounds, Chutes Bloqueados por defensores, trancos (Hits) e roubadas de disco. Goleiros utilizam matemática de _Save Quality_ contra _Shot Quality_ para decidir a física dos gols.
...
### Iteração Alpha 0.1.14 (Phase 5: Age Management & Retirement)

- **Engine de Envelhecimento**: Implementado o motor de avanço biológico (`advanceSeason`). Jogadores da equipe ativa e da coleção envelhecem 1 ano automaticamente no fim do ciclo simulado.
- **Graduação e Aposentadoria Automática (Cap de 22 Anos)**: O ecossistema júnior entra em ação. Jogadores que completam 22 anos são imediatamente removidos do Roster ativo (gelo e banco) e transferidos para um cofre estático, a "Coleção".
- **Sistema de Notificações In-Game**: A UI do Sidebar ganhou um Sino Interativo ("Notification Bell"). Durante a limpeza de temporada, o GM recebe alertas diretos detalhando quais jovens lendas graduaram e deixaram sua franquia.
- **Gacha Temporal (Re-Draft)**: A função `buyPack` da loja foi adaptada. Diferente da trava anterior, os jogadores aposentados (armazenados na Coleção) foram libertados do limbo e reativados no pool de cartas. Comprar um pacote rola o `globalDraftPool` e permite clonar fisicamente a estrela original de 18 anos, gerando o ciclo de renascimento de talentos do "Ultimate Team" e mantendo o legado antigo intocado na Coleção.

### Iteração Alpha 0.1.15 (Phase 6: Sticker Album & Reward System)

- **Painel da Coleção (Sticker Album)**: Substituída a rota de placeholder da Coleção por um hub interativo e horizontal contendo os 20 times iniciais (OHL). A arquitetura renderiza toda a base original da franquia selecionada em formato de Grid estético.
- **Sistema Binário de Posse (CSS Grayscale)**: Cartas renderizadas no álbum cruzam IDs com o `gameState.collection`. Jogadores faltantes sofrem de-saturação em 100% (Preto e Branco) com opacidade morta e um ícone absoluto de Cadeado. Colecionar a lenda acende os gradientes, remove os filtros e acende a opacidade do card, criando forte incentivo de colecionismo.
- **Acompanhamento Fracionado (UI)**: Um _Badge_ conta ativamente o progresso de coleção (ex: 7 / 23 Collected) posicionado no cabeçalho da tabela de time atual, atrelado a um contador geral de jogadores salvos no topo direito da HUD.
- **Engine de Premiação Escalável (FPHL Boost)**: Adicionado a rotina `checkTeamCompletion()` nos ganchos de Aposentadoria Automática e Venda para Coleção. Se todos os IDs originais de um time habitarem a coleção simultaneamente, o sistema sinaliza conclusão e presenteia o jogador imediatamente abrindo um _Reveal Modal_.
- **Pacotes Especiais C-Tier (Placeholder FPHL)**: O prêmio de time completo puxa 2 jogadores randômicos faltantes, multiplica estaticamente todos os seus sub-atributos por `1.5x`, acende sua borda para a Classe `Silver`, aplica a tag `(FPHL)` aos seus nomes e os deposita magicamente no banco do titular ativo, consolidando a migração arquitetural para ligas maiores.
- **Inspeção Premium de Cartas**: A UI foi atualizada permitindo clique direto nas figurinhas coloridas. Acionar uma carta do álbum invoca a renderização do Modal Premium completo do jogador, permitindo visualizar seus stats detalhados de Skating, Shooting e Intelligence da mesma forma que na zona de Roster ativo.

### Iteração Alpha 0.1.16 (Phase 7: Playoff System)

- **Engine Dinâmica de Playoffs**: Criado o módulo `playoffs.js` responsável por calcular os 8 melhores times da Conferência Leste e Oeste ao fim dos 68 jogos, ranqueá-los via pontos (Wins + OTL) e construir automaticamente a árvore do torneio (1v8, 2v7, 3v6, 4v5).
- **Matchmaking 2-2-1-1-1**: A injeção no calendário abraçou a clássica Série Melhor de 7 de Hóquei no Gelo, onde o time mais bem seedado (High Seed) obtém os mandos de campo nos jogos 1, 2, 5 e 7. O motor de simulação descarta matematicamente todos os jogos residuais da série assim que qualquer time atingir 4 vitórias, prosseguindo com o avanço da rodada.
- **Painel Interativo de Chaveamento**: Adicionada uma aba de "Playoffs" oculta que brota na barra lateral quando a temporada regular acaba. O painel lista perfeitamente as disputas ativas da rodada em display gráfico (Matchups), com badges de Seed, os logos dos times e o placar em tempo real (ex: Kitchener leads 2-0).
- **Integração de UI no Dashboard**: A aba _Next Match_ se converte visualmente para a formatação de Playoffs caso a partida programada seja eliminatória, informando qual o Game X da série atual e o seu status geral. Além disso, o botão de "Start Playoffs" encapsula a re-renderização da tela inteira via `switchView`, mantendo as animações orgânicas.
- **Correção da Semântica de Ties**: Finalizando o modelo norte-americano da simulação, os vestígios da string de empates puros ("Ties") foram expurgados para OT LOSS em jogos pulados (skipped). Em partidas renderizadas nos bastidores, a lógica matemática rola corretamente o desempate com Overtime ou Shootouts, acrescentando `+1` direto no placar de gols do vencedor.

### Iteração Alpha 0.1.17 (Season Awards & Player Stats UI)

- **Season Awards & Hall of Fame**: Desenvolvida a lógica visual e computacional de premiações de fim de temporada com base na matemática oficial da liga. Através da função `computeSeasonAwards`, o sistema varre a liga calculando os maiores recordes e entregando troféus nomeados autênticos da OHL (ex: _Red Tilson Trophy_, _Max Kaminsky Trophy_, _J. Ross Robertson Cup_). O histórico é salvo de maneira perene no array `gameState.history` e renderizado numa nova e belíssima aba de "Hall of Fame" localizada no Sidebar.
- **Season Stats na Interface de Cartas**: O Modal Premium do jogador foi aprimorado. Injeção direta de dados esportivos acima da grade estática de atributos, renderizando ativamente uma tabela com os números do jogador durante aquela temporada simulada (GP, G, A, PTS para linha de frente / GP, Saves, SV%, GAA para goleiros).
- **Fallback Visual (Missing Photos)**: Foi importada e configurada nativamente na interface de cartas de jogadores a imagem vetorizada `assets/default-player.svg`. Caso os bancos de dados da CHL falhem na foto oficial de prospectos da base, um evento `onerror` aplica a imagem da sombra humana padrão na carta do jogador sem quebrar a estética premium da UI circular.
- **Correção de Vazamento Estático (Stats Reset)**: Solucionado um erro crítico de _Data Persistence_ que quebrava as tabelas de _League Leaders_. Os stats originais da liga de 2023-2024 que acompanhavam os arrays do Scraper agora são instantaneamente formatados em brancos matemáticos (zeros) tanto no `startGame` (criação da franquia) quanto em `advanceSeason`. Para solidificar o patch, um fix isolou `gameState.globalDraftPool` no armazenamento local (`localStorage`), impedindo que partidas simuladas no gelo percam progressão orgânica da IA ao atualizar a aba do navegador.

### Iteração Alpha 0.1.18 (Advanced Save System & Persistence)

- **Slots de Save Múltiplos**: A arquitetura de _Save_ foi reescrita abandonando a variável única do `localStorage` para suportar 3 slots manuais (`hockeyGmSave_1`, `_2`, `_3`) e um slot automático (`_auto`), garantindo que o progresso do usuário não seja sobreposto por engano.
- **Auto-Save Engine**: Os eventos síncronos de salvar presentes espalhados pelo jogo (após partidas, avanços de temporada, compra de pacotes) agora apontam implicitamente para o slot automático.
- **UI Modals para Gerenciamento de Estado**: Injeção de Modais Glassmorphism escuros tanto para salvar quanto para carregar a franquia. A tela de _Load_ lista perfeitamente as metadados do save (Nome da Equipe e Ano da Temporada) evitando _blind loads_, além de indicar formatações corrompidas e tratar slots vazios visualmente. E também foi corrigido o crash visual (`currentTeam`) de re-injeção do state durante o carregamento das variaveis do HTML.
- **Exportação/Importação de JSON (File Handling)**: Uma solução radical foi implementada para evitar a perda de Saves pelo ecossistema de origens (`origins`) atrelado a portas no Live Server: a interface de _Load/Save_ expõe File Inputs (`<input type="file">`) combinados à API de `Blob`. Agora o GM pode exportar toda a sua franquia para um arquivo `.json` baixado fisicamente na máquina, assim como importá-lo novamente recuperando o jogo perfeitamente.
